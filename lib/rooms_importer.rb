...
class RoomsImporter
  ROOM_PARAMS = [:rmrecnbr, :floor, :room_number, :facility_code_heprod, :rmtyp_description, :dept_id, :dept_grp, :dept_description, :square_feet, :instructional_seating_count, :building_id].freeze
  ...
  def load_rooms_from_csv(file)
    ...
    CSV.foreach(file, headers: true, header_converters: lambda { |header| HEADER_MAP[header] }) do |room|
      if building_exists?(room[:building_id]) && classroom?(room[:rmtyp_description])
  ...
  def set_building_id(room)
    room[:building_id] = room[:bldrecnbr].to_i
  end
